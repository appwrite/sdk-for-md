...
| `size` | `number` | ✅ | Attribute size for varchar attributes, in number of characters. Maximum size is 16381. |
...
const result: Models.AttributeVarchar = await databases.createVarcharAttribute({
...
Returns a `Models.AttributeVarchar` object with the following properties:
