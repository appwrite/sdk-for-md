...
| `passwordCpu` | `number` | ✅ | Optional CPU cost used to hash password. |
| `passwordMemory` | `number` | ✅ | Optional memory cost used to hash password. |
| `passwordParallel` | `number` | ✅ | Optional parallelization cost used to hash password. |
| `passwordLength` | `number` | ✅ | Optional hash length used to hash password. |
...
const result: Models.User = await users.createScryptUser({
  userId: '<USER_ID>',
