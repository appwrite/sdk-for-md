...
| `labels` | `string[]` | ✅ | Array of user labels. Replaces the previous labels. Maximum of 1000 labels are allowed, each up to 36 alphanumeric characters long. |
...
const result: Models.User = await users.updateLabels({
  userId: '<USER_ID>',
  labels: [],
});
```
