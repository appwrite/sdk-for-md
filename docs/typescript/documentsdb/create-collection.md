...
| `attributes` | `object[]` | ❌ | Array of attribute definitions to create. Each attribute should contain: key (string), type (string: string, integer, float, boolean, datetime, relationship), size (integer, required for string type), required (boolean, optional), default (mixed, optional), array (boolean, optional), and type-specific options. (Default: `[]`) |
...
const result: Models.Collection = await documentsDB.createCollection({
