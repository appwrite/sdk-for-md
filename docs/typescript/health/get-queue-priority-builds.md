# getQueuePriorityBuilds

Description: Get the priority builds queue size.

## Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `threshold` | `number` | ❌ | Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 500. (Default: `500`) |

## Usage

```typescript
import { Client, Health, Models } from 'appwrite';

const client = new Client()
    .setEndpoint('https://<REGION>.cloud.appwrite.io/v1').setProject('<YOUR_PROJECT_ID>');

const health = new Health(client);
const result: Models.HealthQueue = await health.getQueuePriorityBuilds({
  threshold: 0,
});
```

## Response Model

Returns a `Models.HealthQueue` object with the following properties:

| Property | Type | Description |
|----------|------|-------------|
| `size` | `number` | Amount of actions in the queue. |
