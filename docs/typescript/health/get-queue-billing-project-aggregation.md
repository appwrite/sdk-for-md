...
|-----------|------|----------|-------------|
| `threshold` | `number` | ❌ | Queue size threshold. When hit (equal or higher), endpoint returns server error. Default value is 5000. (Default: `10000`) |
...
const result: Models.HealthQueue = await health.getQueueBillingProjectAggregation({
  threshold: 0,
});
```
