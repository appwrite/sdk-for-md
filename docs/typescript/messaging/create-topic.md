...
| `subscribe` | `string[]` | ❌ | An array of role strings with subscribe permission. By default all users are granted with any subscribe permission. [learn more about roles](https://appwrite.io/docs/permissions#permission-roles). Maximum of 100 roles are allowed, each 64 characters long. (Default: `[&quot;users&quot;]`) |
...
const result: Models.Topic = await messaging.createTopic({
  topicId: '<TOPIC_ID>',
  name: '<NAME>',
  subscribe: ["any"],
});
