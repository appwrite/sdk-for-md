...
| `type` | `TemplateReferenceType` | ✅ | Type for the reference provided. Can be commit, branch, or tag<br>**Allowed:** `branch`, `commit`, `tag` |
...
```typescript
import { Client, Sites, TemplateReferenceType, Models } from 'appwrite';

const client = new Client()
    .setEndpoint('https://<REGION>.cloud.appwrite.io/v1').setProject('<YOUR_PROJECT_ID>');

const sites = new Sites(client);
const result: Models.Deployment = await sites.createTemplateDeployment({
  siteId: '<SITE_ID>',
