...
| `api` | `Api` | ✅ | API name.<br>**Allowed:** `rest`, `graphql`, `realtime` |
| `status` | `boolean` | ✅ | API status. |

## Usage

```typescript
import { Client, Projects, Api, Models } from 'appwrite';

const client = new Client()
    .setEndpoint('https://<REGION>.cloud.appwrite.io/v1').setProject('<YOUR_PROJECT_ID>');

const projects = new Projects(client);
const result: Models.Project = await projects.updateAPIStatus({
