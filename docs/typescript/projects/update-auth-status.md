...
| `method` | `AuthMethod` | ✅ | Auth Method. Possible values: email-password,magic-url,email-otp,anonymous,invites,jwt,phone<br>**Allowed:** `email-password`, `magic-url`, `email-otp`, `anonymous`, `invites`, `jwt`, `phone` |
| `status` | `boolean` | ✅ | Set the status of this auth method. |

## Usage

```typescript
import { Client, Projects, AuthMethod, Models } from 'appwrite';

const client = new Client()
    .setEndpoint('https://<REGION>.cloud.appwrite.io/v1').setProject('<YOUR_PROJECT_ID>');

const projects = new Projects(client);
const result: Models.Project = await projects.updateAuthStatus({
