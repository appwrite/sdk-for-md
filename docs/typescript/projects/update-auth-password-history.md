...
| `limit` | `number` | ✅ | Set the max number of passwords to store in user history. User can&#039;t choose a new password that is already stored in the password history list.  Max number of passwords allowed in history is20. Default value is 0 |
...
const result: Models.Project = await projects.updateAuthPasswordHistory({
