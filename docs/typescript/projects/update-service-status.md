...
| `service` | `ApiService` | ✅ | Service name.<br>**Allowed:** `account`, `avatars`, `databases`, `tablesdb`, `locale`, `health`, `storage`, `teams`, `users`, `sites`, `functions`, `graphql`, `messaging` |
| `status` | `boolean` | ✅ | Service status. |

## Usage

```typescript
import { Client, Projects, ApiService, Models } from 'appwrite';

const client = new Client()
    .setEndpoint('https://<REGION>.cloud.appwrite.io/v1').setProject('<YOUR_PROJECT_ID>');

const projects = new Projects(client);
const result: Models.Project = await projects.updateServiceStatus({
