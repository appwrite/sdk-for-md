...
const result: Models.BackupArchiveList = await backups.listArchives({
  queries: [],
});
```
...
Returns a `Models.BackupArchiveList` object with the following properties:

| Property | Type | Description |
|----------|------|-------------|
| `total` | `number` | Total number of archives that matched your query. |
| `archives` | `object[]` | List of archives. |
