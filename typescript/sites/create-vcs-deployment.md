...
| `type` | `VCSReferenceType` | ✅ | Type of reference passed. Allowed values are: branch, commit<br>**Allowed:** `branch`, `commit`, `tag` |
...
```typescript
import { Client, Sites, VCSReferenceType, Models } from 'appwrite';

const client = new Client()
    .setEndpoint('https://<REGION>.cloud.appwrite.io/v1').setProject('<YOUR_PROJECT_ID>');

const sites = new Sites(client);
const result: Models.Deployment = await sites.createVcsDeployment({
  siteId: '<SITE_ID>',
