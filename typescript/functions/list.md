...
const result: Models.FunctionList = await functions.list({
  queries: [],
  search: '<SEARCH>',
  total: false,
});
...
Returns a `Models.FunctionList` object with the following properties:

| Property | Type | Description |
|----------|------|-------------|
| `total` | `number` | Total number of functions that matched your query. |
| `functions` | `object[]` | List of functions. |
